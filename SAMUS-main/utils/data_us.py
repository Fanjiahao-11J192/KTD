...
class CropImageToImage2D(Dataset):
    ...
    def __getitem__(self, i):
        ...
        mask_template = (mask == 1)
        crop_image[mask_template] = image[mask_template]
        # if not os.path.isdir(crop_path):
        #     os.makedirs(crop_path)
        # cv2.imwrite(os.path.join(crop_path, filename + '.jpg'), crop_image)
        # correct dimensions if needed
        crop_image, mask = correct_dims(crop_image, mask)
        if self.joint_transform:
            crop_image, mask, low_mask = self.joint_transform(crop_image, mask)
        if self.one_hot_mask:
            assert self.one_hot_mask > 0, 'one_hot_mask must be nonnegative'
            mask = torch.zeros((self.one_hot_mask, mask.shape[1], mask.shape[2])).scatter_(0, mask.long(), 1)
